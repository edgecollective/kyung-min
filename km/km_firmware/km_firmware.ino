...
void setup() {
 ...
 if (! tmp007.begin()) {
  ...
  sCmd.addCommand("PINOUT",     pinoutCommand);  // Converts two arguments to integers and echos them back
...
void forwardCommand(SerialCommand this_sCmd) {
  ...
  this_sCmd.println("Executing stepper forward command");
  arg = this_sCmd.next();
  if (arg != NULL) {
    aNumber = atoi(arg);    // Converts a char string to an integer
    //this_sCmd.print("First argument was: ");
...
void backwardCommand(SerialCommand this_sCmd) {
  ...
  this_sCmd.println("Executing stepper backward command");
  arg = this_sCmd.next();
  if (arg != NULL) {
    aNumber = atoi(arg);    // Converts a char string to an integer
    //this_sCmd.print("First argument was: ");
...
void pinoutCommand(SerialCommand this_sCmd) {
  int aNumber;
  char *arg;
  int myPin=LED_BUILTIN;
  int pinStatus=0;

  this_sCmd.println("Executing pinout command");
  arg = this_sCmd.next();
  if (arg != NULL) {
    aNumber = atoi(arg);    // Converts a char string to an integer
    //this_sCmd.print("First argument was: ");
